...
#  下载视频
def down_video(video_list, title, start_url, page,download_dic):
    num = 1
    print('[正在下载P{}段视频,请稍等...]:'.format(page) + title)
    currentVideoPath = os.path.join(sys.path[0], 'bilibili_video',download_dic, title)  # 当前目录作为下载目录
    for i in video_list:
...
if __name__ == '__main__':
    ...
    # start = input('请输入您要下载的B站Bv号(只要数字)或者视频链接地址:')
    ...
    aid = dec(bid)
    start_url = r'https://api.bilibili.com/x/web-interface/archive/stat?aid=' + str(aid)
    ...
    data = html['data']
    cid_list = data['pages']
    download_filename = bid+data['title']
    for item in cid_list:
        ...
        down_video(video_list, title, start_url, page,download_filename)
        combine_video(video_list, title)

    # 如果是windows系统，下载完成后打开下载目录
    currentVideoPath = os.path.join(sys.path[0], 'bilibili_video',download_filename)  # 当前目录作为下载目录
